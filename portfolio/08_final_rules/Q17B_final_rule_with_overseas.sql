-- ============================================================
-- Q17-B. 최종 최적화 Rule — 해외이상 포함 (4 Rule OR)
-- ============================================================
-- [비즈니스 문제]
--   Q17-A에 해외이상 Rule(4일 이내 국내+해외 동시 사용)을 추가한 버전이다.
--   해외이상 Rule 추가 시 재현율이 향상되지만, 오탐이 일부 발생한다.
--   비즈니스 목표에 따라 Q17-A(오탐 0건) vs Q17-B(재현율 향상) 중 선택한다.
--
--   [4가지 최적화 Rule]
...
--
--   [Q17-A vs Q17-B 비교]
--     방향 A: 오탐 0건 유지, 단 fraud 1명 놓침
--     방향 B: 오탐 7건 추가, fraud 1명 추가 탐지
--     → Q18에서 놓친 고객 검증: is_fraud = TRUE 확인 → 실제 fraud!
--
-- [비즈니스별 최적 선택]
--   한패스/현대카드 (오탐 최소화 우선): Q17-A 선택
--   토스/카카오페이 (재현율 우선):     Q17-B 선택
--
-- [사용 기술]
